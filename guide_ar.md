...
**لماذا C:** ترتيب المهارات يعتمد على الموقع: مستوى المؤسسة يتقدّم على المستوى الشخصي، والمستوى الشخصي يتقدّم على مستوى المشروع. لذلك فإن مهارة شخصية في `~/.claude/skills/commit/SKILL.md` تحجب مهارة الفريق `.claude/skills/commit/SKILL.md` لهذا المطور وحده — لا يتغير أي شيء في المستودع، فيستمر الزملاء في تشغيل نسخة المشروع ويظل سلوك `/commit` كما هو للجميع. الخيار A يعمل من الناحية التقنية لكنه توصية أضعف: يبقى `/commit` الخاص بالمشروع فعّالًا إلى جانب `/my-commit`، ووجود مهارتين بحقلَي `description` شبه متطابقين يجعل اختيار Claude التلقائي للمهارة غير متوقّع، ويكسر العادة المكتسبة ووثائق الفريق التي تنص على تشغيل `/commit`. الخيار B خاطئ لأن frontmatter المهارة لا يوفّر منطقًا شرطيًا ولا وصولًا إلى اسم المستخدم، كما أنه يستلزم تعديل ملف المشروع المشترك وهو ما يستبعده السيناريو. الخيار D خاطئ لأن الحقل `override` غير موجود — فالأولوية تأتي من الموقع لا من تصريح.
