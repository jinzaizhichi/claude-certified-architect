...
**Почему C:** Разрешение скиллов определяется их расположением: корпоративный уровень перекрывает персональный, а персональный — проектный. Поэтому персональный скилл по пути `~/.claude/skills/commit/SKILL.md` перекрывает командный `.claude/skills/commit/SKILL.md` только для этого разработчика — в репозитории ничего не меняется, коллеги продолжают запускать проектную версию, и для всех остальных `/commit` работает по-прежнему. Вариант A технически работает, но это более слабая рекомендация: проектный `/commit` остаётся активным рядом с `/my-commit`, а два скилла с почти одинаковыми полями `description` делают автоматический выбор скилла непредсказуемым и ломают привычку и командную документацию, где сказано запускать `/commit`. Вариант B неверен: во frontmatter скилла нет ни условной логики, ни доступа к имени пользователя, и он потребовал бы правки общего проектного файла, что исключено условием. Вариант D неверен: поля `override` не существует — приоритет задаётся расположением, а не декларацией.
