...
מערך ה-`messages` משתמש בשני תפקידי שיח ותפקיד הנחיה נוסף אחד:
- `user` — הודעות משתמש, כולל תוצאות כלים (נשלחות כ-content block מסוג `tool_result` בתוך הודעה בתפקיד `user`, ולא כתפקיד `tool` נפרד)
- `assistant` — תגובות המודל (כלולות בשליחת היסטוריה), כולל בקשות לשימוש בכלים (content blocks מסוג `tool_use`)
- `system` — ניתן להגדיר דרך השדה העליון `system` (חל מהתור הראשון) או באופן משולב בתוך `messages` כ-`{"role": "system", ...}` (חל מאותה נקודה ואילך, בכפוף לכללי מיקום — ראו בהמשך)

תוצאות כלים אינן נשלחות כהודעה בתפקיד `"tool"`. הן נשלחות כהודעה בתפקיד `user` שתוכנה כולל content block מסוג `tool_result`:
...
`system` יכול להופיע גם כתפקיד ישירות בתוך מערך ה-`messages`, לא רק דרך הפרמטר העליון `system`. מטרת הדבר היא להוסיף הנחיות באמצע השיחה מבלי לבטל את ה-prefix השמור במטמון (cache) של השדה העליון `system`. לכך יש כללי מיקום מסוימים:
- חייב לבוא מיד אחרי תור `user` (כולל כזה שמכיל בלוקי `tool_result`) או תור `assistant` שמסתיים בשימוש בכלי מצד השרת.
- חייב לקדום תור `assistant` או להיות האחרון במערך.
- אינו יכול לשבת בין בלוק `tool_use` ל-`tool_result` שלו — עשיית כך מחזירה שגיאת 400.
- הודעות `system` מאוחרות יותר (כולל כאלה שבאמצע השיחה) גוברות על קודמות ועל השדה העליון `system` עבור התורות הבאים.
